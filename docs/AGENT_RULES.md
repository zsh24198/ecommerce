...
- 错误码分段（与 HTTP 状态码解耦，便于业务扩展）：
  - `1000-1999` 通用客户端错误（参数校验失败 1001、请求体解析失败 1002）
  - `2000-2999` 认证授权错误（token 无效 2001、token 过期 2002、权限不足 2003）
  - `3000-3999` 服务端内部错误（数据库连接失败 3001、未知错误 3002）
  - `4000-4999` 第三方依赖错误（Redis 超时 4001、Kafka 投递失败 4002）
  - `5000-5999` 业务逻辑错误（库存不足 5001、订单状态非法 5002、用户已存在 5003）
  - `6000-6999` 限流熔断错误（触发限流 6001、服务降级 6002）
- 每个错误码必须定义在 apperror 包的常量中，禁止散落魔法数字
